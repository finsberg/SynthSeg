...
def build_qc_model(
    ...
    last = KL.MaxPool3D(pool_size=(2, 2, 2), name="qc_maxpool_%s" % (n_levels - 1), padding="same")(
        last
    )
...
def build_qc_loss(input_model):
    ...
    # get loss
    loss = KL.Lambda(lambda x: K.sum(K.mean(K.square(x[0] - x[1]), axis=0)), name="qc_loss")(
        [dice_gt, dice_pred]
    )
...
def get_shapes(labels_shape, cropping_shape, output_div_by_n, n_dims):
    ...
    if cropping_shape is not None:
        ...
        if output_div_by_n is not None:
            tmp_shape = [
                utils.find_closest_number_divisible_by_m(s, output_div_by_n) for s in cropping_shape
            ]
    ...
    # no cropping shape specified, so no cropping unless label_shape is not divisible by output_div_by_n
        ...
        if output_div_by_n is not None:
            cropping_shape = [
                utils.find_closest_number_divisible_by_m(s, output_div_by_n) for s in labels_shape
            ]
...
def train_model(
    ...
    if (path_checkpoint is not None) & (not reinitialise_momentum):
        ...
        custom_l2i = {
            key: value for (key, value) in getmembers(l2i_layers, isclass) if key != "Layer"
        }
        custom_nrn = {
            key: value for (key, value) in getmembers(nrn_layers, isclass) if key != "Layer"
        }
        custom_objects = {
            **custom_l2i,
            **custom_nrn,
...
class SimulatePartialFOV(KL.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        x = K.switch(
            tf.squeeze(K.greater(tf.random.uniform([1], 0, 1), 1 - self.prob_mask)), x * mask, x
        )
