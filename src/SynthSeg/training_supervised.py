...
def training(
    ...
    train_model(
        dice_model, input_generator, lr, dice_epochs, steps_per_epoch, model_dir, "dice", checkpoint
    )
...
def build_augmentation_model(
    ...
    if flipping:
        ...
        labels, image = layers.RandomFlip(
            get_ras_axes(aff, n_dims)[0], [True, False], segmentation_labels, n_neutral_labels
        )([labels, image])
    ...
    if data_res is not None:
        ...
        for i, channel in enumerate(split):
            if randomise_res:
                ...
                channel = layers.DynamicGaussianBlur(0.75 * max_res / np.array(atlas_res), 1.03)(
                    [channel, sigma]
                )
        ...
        image = (
            KL.Lambda(lambda x: tf.concat(x, -1))(channels) if len(channels) > 1 else channels[0]
        )
