...
def blurring_sigma_for_downsampling(current_res, downsample_res, mult_coef=None, thickness=None):
    ...
    if not tf.is_tensor(downsample_res):
        ...
        if mult_coef is None:
            sigma = KL.Lambda(
                lambda x: tf.where(
                    tf.math.equal(x, tf.convert_to_tensor(current_res, dtype="float32")),
                    0.5,
            ...
            sigma = KL.Lambda(
                lambda x: mult_coef * x / tf.convert_to_tensor(current_res, dtype="float32")
            )(down_res)
...
def unit_kernel(dist_threshold, n_dims, max_dist_threshold=None):
    ...
    # get batchsize
    batchsize = (
        None if shape[0] is not None else tf.split(tf.shape(dist_threshold_tens), [1, -1])[0]
    )
    ...
    kernel = tf.where(
        tf.less_equal(dist, dist_threshold_tens), tf.ones_like(dist), tf.zeros_like(dist)
    )
...
def resample_tensor(
    ...
    if subsample_res is not None:
        ...
        assert len(subsample_res) == len(volume_res), (
            "subsample_res and volume_res must have the same length, " "had {0}, and {1}".format(
