...
def lab2im_model(
    ...
    image = layers.BiasFieldCorruption(0.3, 0.025, same_bias_for_all_channels=False)(image)
...
def get_shapes(labels_shape, output_shape, atlas_res, target_res, output_div_by_n):
    ...
    if output_shape is not None:
        ...
        if output_div_by_n is not None:
            tmp_shape = [
                utils.find_closest_number_divisible_by_m(s, output_div_by_n) for s in output_shape
            ]
        ...
        if resample_factor is not None:
            cropping_shape = [
                int(np.around(output_shape[i] / resample_factor[i], 0)) for i in range(n_dims)
            ]
        else:
    ...
    # no output shape specified, so no cropping unless label_shape is not divisible by output_div_by_n
        ...
        if resample_factor is not None:
            output_shape = [
                int(np.around(cropping_shape[i] * resample_factor[i], 0)) for i in range(n_dims)
            ]
        else:
        ...
        if output_div_by_n is not None:
            output_shape = [
                utils.find_closest_number_divisible_by_m(s, output_div_by_n, answer_type="closer")
