...
def rescale_volume(
    volume, new_min=0, new_max=255, min_percentile=2, max_percentile=98, use_positive_only=False
):
    ...
    robust_min = (
        np.min(intensities) if min_percentile == 0 else np.percentile(intensities, min_percentile)
    )
    robust_max = (
        np.max(intensities) if max_percentile == 100 else np.percentile(intensities, max_percentile)
    )
...
def correct_label_map(
    ...
    assert (
        (list_correct_labels is not None) | use_nearest_label
                ...
                # several possibilities
                    ...
                    while correct_label_not_found:
                        tmp_labels, crop = crop_volume_around_region(
                            labels, masking_labels=incorrect_label, margin=10 * margin_mult
                        )
                    ...
                    distance_map_list = [
                        distance_transform_edt(tmp_labels != lab) for lab in correct_label
                    ]
                ...
                for i in range(1, n_components + 1):
                    ...
                    if len(correct_labels) == 0:
                        ...
                        if len(correct_labels) == 1:
                            ...
                            distance_map_list = [
                                distance_transform_edt(tmp_labels != lab) for lab in correct_labels
                            ]
                            distances_correct = np.stack(
                                [dist[incorrect_voxels] for dist in distance_map_list]
                            )
                    ...
                    if n_dims == 2:
                        ...
                        new_labels[crop[0] : crop[3], crop[1] : crop[4], crop[2] : crop[5], ...] = (
                            tmp_new_labels
                        )
...
def erode_label_map(
    labels, labels_to_erode, erosion_factors=1.0, gpu=False, model=None, return_model=False
):
    ...
    for label_to_erode, erosion_factor in zip(labels_to_erode, erosion_factors):
        ...
        list_dist_maps = [
            distance_transform_edt(np.logical_not(cropped_labels == la)) for la in labels_list
        ]
        ...
        if n_dims == 2:
            ...
            new_labels[
                cropping[0] : cropping[3], cropping[1] : cropping[4], cropping[2] : cropping[5], ...
            ] = cropped_labels
...
def compute_hard_volumes(labels, voxel_volume=1.0, label_list=None, skip_background=True):
    ...
    for idx, (path_image, path_mask) in enumerate(zip(path_images, path_masks)):
        ...
        if (not os.path.isfile(path_result)) | recompute:
            ...
            if path_mask is not None:
                ...
                mask = None
            im = mask_volume(
                im, mask, threshold, dilate, erode, fill_holes, masking_value, write_mask
            )
        ...
        if (not os.path.isfile(path_result)) | recompute:
            ...
            if gpu:
                ...
                if mask is None:
                    ...
                    im = np.squeeze(
                        model.predict([utils.add_axis(im, [0, -1]), utils.add_axis(mask, [0, -1])])
                    )
            else:
...
def convert_images_in_dir_to_nifty(
    ...
    for idx, (path_image, path_ref) in enumerate(zip(path_images, path_ref_images)):
        ...
        path_result = (
            os.path.join(result_dir, os.path.basename(utils.strip_extension(path_image)))
            + ".nii.gz"
        )
        if (not os.path.isfile(path_result)) | recompute:
    ...
    for idx, path_image in enumerate(path_images):
        ...
        if keep_segm_only:
            path_result = os.path.join(
                result_dir, utils.strip_extension(os.path.basename(path_image)) + "_seg.mgz"
            )
        else:
        ...
        if (not os.path.isfile(path_result)) | recompute:
            cmd = utils.mkcmd(
                path_samseg, "-i", path_image, "-o", path_im_result_dir, "--threads", threads
            )
...
def upsample_anisotropic_images(
    ...
    for idx, (path_image, path_ref) in enumerate(zip(path_images, path_ref_images)):
        ...
        if (not os.path.isfile(path_im_upsampled)) | recompute:
            cmd = utils.mkcmd(
                mri_convert, path_image, path_im_upsampled, "-rl", path_ref, "-odt float"
            )
            os.system(cmd)
        ...
        if (not os.path.isfile(path_dist_map)) | recompute:
            ...
            for i, maps in enumerate(dist_map):
                ...
                cmd = utils.mkcmd(
                    mri_convert, path_mesh, path_mesh_up, "-rl", path_im_upsampled, "-odt float"
                )
                os.system(cmd)
...
def simulate_upsampled_anisotropic_images(
    ...
    path_images = utils.list_images_in_folder(image_dir)
    path_labels = (
        [None] * len(path_images) if labels_dir is None else utils.list_images_in_folder(labels_dir)
    )
    ...
    for idx, (path_image, path_labels) in enumerate(zip(path_images, path_labels)):
        ...
        if (not os.path.isfile(path_im_upsampled)) | recompute:
            cmd = utils.mkcmd(
                mri_convert, path_im_downsampled, path_im_upsampled, "-rl", path_image, "-odt float"
            )
            os.system(cmd)
        ...
        if build_dist_map:
            ...
            if (not os.path.isfile(path_dist_map)) | recompute:
                ...
                for i, d_map in enumerate(dist_map):
                    ...
                    cmd = utils.mkcmd(
                        mri_convert, path_mesh, path_mesh_up, "-rl", path_image, "-odt float"
                    )
                    os.system(cmd)
                    path_meshes_up.append(path_mesh_up)
                mesh_up_0, aff, h = utils.load_volume(path_meshes_up[0], im_only=False)
    ...
    for idx, path_label in enumerate(path_labels):
        ...
        if (not os.path.isfile(path_result)) | recompute:
            ...
            im = correct_label_map(
                im, incorrect_labels, correct_labels, use_nearest_label, remove_zero, smooth
            )
            utils.save_volume(im, aff, h, path_result)


def mask_labels_in_dir(
    labels_dir, result_dir, values_to_keep, masking_value=0, mask_result_dir=None, recompute=True
):
...
def smoothing_gpu_model(label_shape, label_list, connectivity=1):
    ...
    labels = KL.Lambda(lambda x: tf.one_hot(tf.cast(x, dtype="int32"), depth=n_labels, axis=-1))(
        labels
    )
...
def erode_labels_in_dir(
    labels_dir, result_dir, labels_to_erode, erosion_factors=1.0, gpu=False, recompute=True
):
...
def build_atlas(
    labels_dir, label_list, align_centre_of_mass=False, margin=15, shape=None, path_atlas=None
):
    ...
    for idx, path_label in enumerate(path_labels):
        ...
        if align_centre_of_mass:
            centre_of_mass = np.array(
                [np.mean(indices[0]), np.mean(indices[1]), np.mean(indices[2])], dtype="int32"
            )
            min_crop = centre_of_mass - margin
            max_crop = centre_of_mass + margin
            atlas += lab[
                min_crop[0] : max_crop[0], min_crop[1] : max_crop[1], min_crop[2] : max_crop[2], ...
            ]
...
def crop_dataset_to_minimum_size(
    ...
    for idx, (path_label, path_image) in enumerate(zip(path_labels, path_images)):
        ...
        if path_image is not None:
            ...
            utils.save_volume(
                image, aff_im, h_im, os.path.join(image_result_dir, os.path.basename(path_image))
            )
...
def crop_dataset_around_region_of_same_size(
    labels_dir, result_dir, image_dir=None, image_result_dir=None, margin=0, recompute=True
):
    # create result dir
    utils.mkdir(result_dir)
    if image_dir is not None:
    ...
    if (image_dir is not None) & (not recompute_labels):
        recompute_labels = any(
            [
                not os.path.isfile(os.path.join(image_result_dir, os.path.basename(path)))
