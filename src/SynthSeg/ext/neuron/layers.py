...
class SpatialTransformer(Layer):
    ...
    def build(self, input_shape):
        ...
        for i, shape in enumerate(trf_shape):
            ...
            self.is_affine.append(
                len(shape) == 1 or (len(shape) == 2 and all([f == (self.ndims + 1) for f in shape]))
            )
            ...
            if not self.is_affine[i]:
                if shape[-1] != self.ndims:
                    raise Exception(
                        "Offset flow field size expected: %d, found: %d" % (self.ndims, shape[-1])
                    )
    ...
    def call(self, inputs, **kwargs):
        ...
        if len(trf) == 1:
            ...
            if self.is_affine[0]:
                trf = tf.map_fn(
                    lambda x: self._single_aff_to_shift(x, vol.shape[1:-1]), trf, dtype=tf.float32
                )
...
class Resize(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        if not any(self.zoom_factor0):
            ...
            self.size0 = [
                int(self.inshape[f + 1] * self.zoom_factor0[f]) for f in range(self.ndims)
            ]
