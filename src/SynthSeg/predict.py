...
def predict(
    ...
    (
        ...
        path_images, path_segmentations, path_posteriors, path_resampled, path_volumes, recompute
    )
...
def build_model(
    ...
    if gradients:
        ...
        last_tensor = KL.Lambda(lambda x: (x - K.min(x)) / (K.max(x) - K.min(x) + K.epsilon()))(
            last_tensor
        )
    ...
    if flip_indices is not None:
        ...
        last_tensor = KL.Lambda(lambda x: tf.split(x, [1] * n_labels_seg, axis=-1), name="split")(
            last_tensor
        )
...
def get_flip_indices(labels_segmentation, n_neutral_labels):
    ...
    lr_corresp_unique = (
        lr_corresp_unique[:, 1:] if not np.all(lr_corresp_unique[:, 0]) else lr_corresp_unique
    )
