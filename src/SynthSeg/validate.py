...
def plot_validation_curves(
    ...
    if eval_indices is not None:
        if isinstance(eval_indices, (np.ndarray, str)):
            ...
            eval_indices = np.squeeze(
                utils.reformat_to_n_channels_array(eval_indices, n_dims=len(eval_indices))
            )
    ...
    if plot_legend is False:
        ...
        list_legend_labels = architecture_names
        list_legend_labels = [
            "_nolegend_" if i >= plot_legend else list_legend_labels[i] for i in range(n_curves)
        ]
    ...
    for idx, (net_val_dir, net_name, linestyle, colour, legend_label, eval_idx) in enumerate(
        zip(
            list_validation_dirs,
            architecture_names,
            list_linestyles,
        ...
        if list_net_scores:  # check that archi has been validated for at least 1 epoch
            ...
            plt.plot(
                list_epochs, list_net_scores, label=legend_label, linestyle=linestyle, color=colour
            )
