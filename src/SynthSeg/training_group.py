...
class BrainGeneratorGroup(BrainGenerator):
    def __init__(self, grouping_labels=None, **kwargs):
        ...
        self.labels_to_image_model, self.model_output_shape = (
...
def labels_to_image_model_group(
    ...
    grouped_labels = KL.Lambda(lambda x: tf.one_hot(tf.cast(x[..., 0], "int32"), n_group, axis=-1))(
        grouped_labels
    )
    split = KL.Lambda(lambda x: tf.split(x, [1] * n_group, axis=-1))(grouped_labels)
    channels = [
        layers.RandomDilationErosion(1, 1, 5, 0.9, operation="random", return_mask=True)(c)
        for c in split
    ]
    ...
    if flipping:
        ...
        labels, grouped_labels = layers.RandomFlip(
            get_ras_axes(aff, n_dims)[0], [True, False], generation_labels, n_neutral_labels
    ...
    labels = layers.ConvertLabels(generation_labels, dest_values=output_labels, name="labels_out")(
        labels
    )

    # build model (dummy layer enables to keep the labels when plugging this model to other models)
    image = KL.Lambda(
        lambda x: tf.concat([x[0], tf.cast(x[1], dtype=x[0].dtype)], axis=-1), name="image_out"
