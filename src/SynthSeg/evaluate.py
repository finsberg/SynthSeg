...
def fast_dice(x, y, labels):
    ...
    if len(labels) > 1:
        ...
        label_edges = np.insert(
            label_edges, [0, len(label_edges)], [labels_sorted[0] - 0.1, labels_sorted[-1] + 0.1]
        )
...
def surface_distances(x, y, hausdorff_percentile=None, return_coordinate_max_distance=False):
    ...
    for hd_percentile in hausdorff_percentile:
        ...
        # find percentile of max distance
            ...
            max_dist.append(
                np.percentile(np.concatenate([x_dists_to_y, y_dists_to_x]), hd_percentile)
            )
...
def evaluation(
    ...
    compute_hausdorff_99 = (
        not os.path.isfile(path_hausdorff_99) if (path_hausdorff_99 is not None) else False
    )
    compute_hausdorff_95 = (
        not os.path.isfile(path_hausdorff_95) if (path_hausdorff_95 is not None) else False
    )
    compute_mean_dist = (
        not os.path.isfile(path_mean_distance) if (path_mean_distance is not None) else False
    )
