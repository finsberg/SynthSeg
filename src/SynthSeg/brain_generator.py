    ...
    def __init__(
        ...
        if subjects_prob is not None:
            self.subjects_prob = np.array(
                utils.reformat_to_list(subjects_prob, load_as_numpy=True), dtype="float32"
            )
            assert len(self.subjects_prob) == len(self.labels_paths), (
                "subjects_prob should have the same length as labels_path, " "had {} and {}".format(
                    len(self.subjects_prob), len(self.labels_paths)
                )
            )
        else:
