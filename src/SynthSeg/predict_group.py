...
def predict(
    ...
    (
        ...
        path_images, path_masks, path_segmentations, path_posteriors, path_volumes, recompute
    )
    ...
    for i in range(len(path_images)):
        ...
        if compute[i]:
            ...
            if path_volumes[i] is not None:
                ...
                write_csv(
                    path_volumes[i], row, unique_vol_file, labels_segmentation, names_segmentation
                )
...
def prepare_output_files(path_images, path_masks, out_seg, out_posteriors, out_volumes, recompute):
    ...
    if basename[-4:] == ".txt":
        ...
        if not os.path.isfile(path_masks):
            raise Exception(
                "provided text file containing paths of input images does not exist" % path_masks
            )
    ...
    # path_images is a folder
        ...
        if os.path.isfile(path_masks):
            raise Exception(
                "Extension not supported for %s, only use: nii.gz, .nii, .mgz, or .npz" % path_masks
            )
    ...
    recompute_list = [
        ...
        for (re_seg, re_post, re_vol) in zip(recompute_seg, recompute_post, recompute_volume)
    ]
    ...
    if n_channels == 1:
        ...
        for i in range(im.shape[-1]):
            im[..., i] = edit_volumes.rescale_volume(
                im[..., i], new_min=0.0, new_max=1.0, min_percentile=0.5, max_percentile=99.5
            )
...
def build_model(
    ...
    labels = KL.Lambda(lambda x: tf.one_hot(tf.cast(x, "int32"), depth=len(mask_labels), axis=-1))(
        input_labels
    )
