...
def prepare_output_files(path_predictions, out_corrections, out_posteriors, out_volumes, recompute):
    ...
    recompute_list = [
        ...
        for (re_cor, re_post, re_vol) in zip(recompute_cor, recompute_post, recompute_volume)
    ]
...
def preprocess(path_prediction, n_levels, crop=None, min_pad=None):
    ...
    if crop is not None:
        ...
        crop_idx = None
        input_shape = [
            utils.find_closest_number_divisible_by_m(s, 2**n_levels, "higher") for s in shape
        ]
...
def build_model(
    ...
    labels = KL.Lambda(
        lambda x: tf.one_hot(tf.cast(x, dtype="int32"), depth=len(input_label_list), axis=-1)
    )(labels)
