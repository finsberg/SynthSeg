...
def sample_segmentation_pairs(
    ...
    while i <= n_examples:
        ...
        for output, name, res_dir in zip(
            outputs, ["labels_gt", "labels_pred_argmax_convert"], [gt_result_dir, pred_result_dir]
        ):
...
def build_augmentation_model(
    ...
    image = layers.IntensityAugmentation(noise_std, gamma_std=gamma_std, contrast_inversion=True)(
        image
    )
    ...
    for i, channel in enumerate(split):
        ...
        channel = layers.DynamicGaussianBlur(0.75 * max_res / np.array(atlas_res), blur_range)(
            [channel, sigma]
        )
        channel = layers.MimicAcquisition(atlas_res, min_res, im_shape, False, noise_std_lr)(
            [channel, resolution]
        )
