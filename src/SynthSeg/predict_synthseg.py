...
def predict(
    ...
    if len(path_segmentations) == 1:  # only one image is processed
        ...
        if (
            len(
                set(
                    [os.path.dirname(path_segmentations[i]) for i in range(len(path_segmentations))]
                )
            )
            <= 1
        ):
...
def prepare_output_files(
    ...
    out_posteriors = (
        os.path.abspath(out_posteriors) if (out_posteriors is not None) else out_posteriors
    )
    ...
    recompute_list = [
        ...
        for (re_seg, re_post, re_res, re_vol, re_qc) in zip(
            recompute_seg, recompute_post, recompute_resampled, recompute_volume, recompute_qc
        )
    ]
...
def preprocess(
    path_image, ct, target_res=1.0, n_levels=5, crop=None, min_pad=None, path_resample=None
):
    ...
    if crop is not None:
        ...
        crop_shape = [
            utils.find_closest_number_divisible_by_m(s, 2**n_levels, "higher") for s in crop
        ]
...
def build_model(
    ...
    if robust:
        ...
        last_tensor = KL.Lambda(lambda x: tf.one_hot(tf.cast(x, "int32"), depth=n_groups, axis=-1))(
            last_tensor
        )
        ...
        if flip_indices is not None:
            ...
            last_tensor = KL.Lambda(lambda x: 0.5 * (x[0] + x[1]), name=name_segm_prediction_layer)(
                [seg, last_tensor]
            )
            net = Model(inputs=net.inputs, outputs=last_tensor)
    ...
    if do_parcellation:
        ...
        last_tensor = layers.ConvertLabels(np.arange(n_labels_seg), labels_segmentation)(
            last_tensor
        )
        parcellation_masking_values = np.array(
            [1 if ((ll == 3) | (ll == 42)) else 0 for ll in labels_segmentation]
        )
        last_tensor = layers.ConvertLabels(labels_segmentation, parcellation_masking_values)(
            last_tensor
        )
        last_tensor = KL.Lambda(lambda x: tf.one_hot(tf.cast(x, "int32"), depth=2, axis=-1))(
            last_tensor
        )
        last_tensor = KL.Lambda(lambda x: tf.cast(tf.concat(x, axis=-1), "float32"))(
            [input_image, last_tensor]
    ...
    if do_qc:
        ...
        if do_parcellation:
            last_tensor = KL.Lambda(lambda x: tf.cast(tf.argmax(x[0], axis=-1), "int32"))(
                net.outputs
            )
        else:
        ...
        last_tensor = KL.MaxPool3D(pool_size=(2, 2, 2), name="qc_maxpool_3", padding="same")(
            last_tensor
        )
        last_tensor = KL.Conv3D(n_labels_qc, kernel_size=5, **conv_kwargs, name="qc_final_conv_0")(
            last_tensor
        )
        last_tensor = KL.Conv3D(n_labels_qc, kernel_size=5, **conv_kwargs, name="qc_final_conv_1")(
            last_tensor
        )
        last_tensor = KL.Lambda(lambda x: tf.reduce_mean(x, axis=[1, 2, 3]), name="qc_final_pred")(
            last_tensor
        )
...
def postprocess(
    ...
    if crop_idx is not None:
        ...
        seg[crop_idx[0] : crop_idx[3], crop_idx[1] : crop_idx[4], crop_idx[2] : crop_idx[5]] = (
            seg_patch
        )
        posteriors[
